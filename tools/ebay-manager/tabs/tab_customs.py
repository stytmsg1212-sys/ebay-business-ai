...
# (st.container(key="customs_root") 内側 div に付く class="st-key-customs_root"
# を掴む。他タブ / app.py のグローバル密度 CSS には触れない = K2 surgical)。
# user 承認済み密度スペック: フォント12px / 行高22-28px / expander既定閉 /
# 常時caption→help tooltip化。
_CUSTOMS_DENSITY_CSS = """<style>
div[class*="st-key-customs_root"] [data-testid="stMarkdownContainer"] p {
    font-size: 12px !important;
    line-height: 24px !important;
    margin: 2px 0 !important;
}
div[class*="st-key-customs_root"] [data-testid="stCaptionContainer"] p {
    font-size: 11px !important;
    line-height: 20px !important;
    margin: 2px 0 !important;
}
div[class*="st-key-customs_root"] [data-testid="stExpander"] details > summary {
    padding: 3px 8px !important;
    min-height: 24px !important;
}
div[class*="st-key-customs_root"] [data-testid="stExpander"] summary p,
div[class*="st-key-customs_root"] [data-testid="stExpander"] summary span {
    font-size: 12px !important;
    line-height: 22px !important;
    margin: 0 !important;
}
div[class*="st-key-customs_root"] [data-testid="stAlert"] {
    padding: 6px 10px !important;
}
div[class*="st-key-customs_root"] [data-testid="stCodeBlock"] pre {
    padding: 6px 8px !important;
    font-size: 12px !important;
    line-height: 22px !important;
}
div[class*="st-key-customs_root"] [data-testid="stButton"] > button {
...
def render_customs_tab() -> None:
    ...
    # C2 密度化 (2026-07-04): with 文でタブ全体を customs_root コンテナに
    # 収め、上記スコープ CSS を効かせる (K2 surgical、ロジック・データ変更なし)。
    with st.container(key="customs_root"):
        st.markdown(_CUSTOMS_DENSITY_CSS, unsafe_allow_html=True)

        # ── 最新情報取得ボタン ──
        # daily_scheduler の 06:10 朝バッチを待たずに、user 操作で即時 Gmail から
        # 通関メールを再取得する. 過去 7 日範囲を再 scan, 新規のみ DB に追加 (gmail_id UNIQUE).
        # C2 密度化: 常時表示 caption (前回自動取得: 毎朝06:10) → ボタン help へ統合、
        # 右側の空き列 (旧 col_b) を廃止 (全幅引き伸ばし禁止・内容幅左詰め)。
        ...
        with _cust_tab_pending:
            ...
            for _req in _pending:
                ...
                _header = (
                    f"<b>[{_req['carrier'].upper()}]</b> "
                    f"TRK# {_req.get('tracking_number') or '(不明)'}  "
                    f'<span style="color:#2e7d5b;">'
                    f'{_req.get("product_title") or "(商品未特定)"}</span>'
                )
                st.markdown(
