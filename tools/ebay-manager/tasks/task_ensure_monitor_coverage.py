...
設計上の不変条件 (Codex review 反映):
  ...
  - 無在庫SKU は「supplier item id → source_url」が一意対応のため、
    同 source_url で別 sku の衝突は構造的に起きない (NOT EXISTS by sku で
    対象抽出 → upsert_item が ebay_item_id 一致行を UPDATE = sku drift 訂正)。
  ...
  - 冪等: 登録済は対象に入らない (NOT EXISTS by sku)。2 回連続実行で
    registered=0。
"""
