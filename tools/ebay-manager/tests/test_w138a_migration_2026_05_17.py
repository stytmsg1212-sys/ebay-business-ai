...
Q2 db-migration-rules:
  - fresh DB → init_db で 2 列が ebay_listings に存在 + user_version=48
...
def test_v41_columns_exist_and_version(tmp_db):
    ...
    with get_conn() as c:
        ...
        assert ver == 48, f"user_version={ver} (期待 48 = HEAD: v41 W138-A → v42 W7/W183 → v43 W142 +each → v44 W140 メモ → v45 W133-FU fulfillment_kind → v46 W148 キーワード新着監視 → v47 W149 eBay 売却注文取得 + fulfillment ひも付け → v48 W149 sales_history 複合 UNIQUE INDEX)"
...
def test_v41_idempotent_data_preserved(tmp_db):
    ...
    with get_conn() as c:
        ver = c.execute("PRAGMA user_version").fetchone()[0]
        assert ver == 48, f"version drift: {ver}"
        row = c.execute(
...
def test_v41_alter_idempotent_no_crash_on_repeat(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(db_path) as c:
        ver = c.execute("PRAGMA user_version").fetchone()[0]
        assert ver == 48  # v41→v42→v43(W142)→v44(W140)→v45(W133-FU)→v46(W148)→v47(W149)→v48(W149 silent gap fix) まで到達 (canonical HEAD)
