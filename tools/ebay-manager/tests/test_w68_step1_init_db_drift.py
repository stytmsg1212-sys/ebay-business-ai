...
def test_v29_supplier_eval_pending_idempotent_fresh(tmp_path, monkeypatch):
    ...
    with db_mod.get_conn() as c:
        ...
        assert ver == 46  # canonical HEAD (v41 W138-A→v42 W7/W183→v43 W142 +each→v44 W140 メモ→v45 W133-FU fulfillment_kind→v46 W148 キーワード新着監視)
...
def test_init_db_idempotent_v27_existing_db(tmp_db):
    ...
    with get_conn() as c:
        ...
        assert ver_before == ver_after == 46, (
            f"user_version drift: before={ver_before} after={ver_after} "
            "(期待 46 維持: v46 = W148 キーワード新着監視 canonical HEAD, "
            "v41 W138-A → v42 W7/W183 → v43 W142 +each → v44 W140 メモ "
            "→ v45 W133-FU fulfillment_kind の後)"
        )
...
def test_init_db_fresh_env_creates_new_schema(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(db_path) as c:
        ...
        ver = c.execute("PRAGMA user_version").fetchone()[0]
        assert ver == 46, f"user_version != 46: {ver} (期待 46 = HEAD: v41 W138-A → v42 W7/W183 → v43 W142 +each → v44 W140 メモ → v45 W133-FU fulfillment_kind → v46 W148 キーワード新着監視)"
