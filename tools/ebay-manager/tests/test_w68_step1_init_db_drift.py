...
def test_v29_supplier_eval_pending_idempotent_fresh(tmp_path, monkeypatch):
    ...
    with db_mod.get_conn() as c:
        ...
        assert ver >= 60  # canonical HEAD (...→v59 W206 keyword_watches.ebay_item_id→v60 W209 news_action_reports)
...
def test_init_db_idempotent_v27_existing_db(tmp_db):
    ...
    with get_conn() as c:
        ...
        assert ver_before == ver_after and ver_after >= 60, (
            f"user_version drift: before={ver_before} after={ver_after} "
            "(期待 >=60 維持: v60 = W209 news_action_reports canonical HEAD)"
        )
...
def test_init_db_fresh_env_creates_new_schema(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(db_path) as c:
        ...
        ver = c.execute("PRAGMA user_version").fetchone()[0]
        assert ver >= 60, f"user_version < 60: {ver} (期待 >=60 = HEAD: ...→ v57 health autofix → v58 W192 Yahoo site_config → v59 W206 keyword_watches.ebay_item_id → v60 W209 news_action_reports)"
